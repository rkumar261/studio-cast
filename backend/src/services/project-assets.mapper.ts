...
import type { ParticipantMasterStatus } from './participant-asset.service.js';
import type { LoadedProjectAssetsRecording } from './project-assets.loader.js';
...
function mapTranscriptState(state?: transcript_state | null): ProjectAssetState {
  if (!state) return 'processing';
  if (state === 'ready') return 'ready';
  if (state === 'failed') return 'action required';
  return 'processing';
...
/**
 * Converts the raw Prisma result plus participant master states into the stable
 * project workspace asset graph consumed by the frontend.
 */
export function buildProjectAssetsGraph(args: {
  recording: LoadedProjectAssetsRecording;
  participantMasterStates: ParticipantMasterStatus[];
}): GetProjectAssetsGraphResponse {
  const { recording, participantMasterStates } = args;

  const rawPreviewByParticipant = new Map<string, string>();
  for (const participant of recording.participant) {
    const rawKey = participant.track[0]?.storage_key_raw;
    const rawUrl = toPublicAssetUrl(rawKey);
    if (rawUrl) rawPreviewByParticipant.set(participant.id, rawUrl);
  }
  ...
  const participantAssets = applicableParticipants.map((participant) => {
    ...
    return {
      ...
      qualityWarnings: (() => {
        const warnings = participant.asset?.metadata?.qualityWarnings as Record<string, unknown> | undefined;
        if (!warnings) return undefined;
        return {
          audioWarning: typeof warnings.audioWarning === 'string' ? warnings.audioWarning : undefined,
          videoWarning: typeof warnings.videoWarning === 'string' ? warnings.videoWarning : undefined,
          durationWarning: warnings.durationWarning === true ? true : undefined,
        };
      })(),
    };
  });
  ...
  const exportItems = [
    ...
    const blockedReason = blockedReasonForExport({
      state,
    ...
    return {
      ...
      pendingWork: isPendingState(state)
        ? [toWorkItem({ label: entry.label, state, reason: blockedReason })!]
        : [],
  ...
  const data: GetProjectAssetsGraphResponse = {
    ...
    exports: {
      ...
      ready: exportItems.filter((item) => item.state === 'ready').length,
      processing: exportItems.filter((item) => item.state === 'processing').length,
      actionRequired: exportItems.filter((item) => item.state === 'action required').length,
      items: exportItems,
    },
  };
