...
const mobileNavItems: SidebarItem[] = [
  desktopSidebarItems[0],
  desktopSidebarItems[1],
  {
    label: 'Upload',
...
export default function WorkspaceSidebar() {
  ...
  useEffect(() => {
    function handlePointerDown(event: MouseEvent) {
      ...
      const insideDesktop = desktopAccountMenuRef.current?.contains(target);
  ...
  function renderAccountPopover(position: 'desktop' | 'mobile') {
    const positionClassName =
      position === 'desktop'
        ? 'absolute bottom-0 left-[calc(100%+16px)] z-[110] w-72'
        : 'absolute bottom-full right-0 z-[110] mb-3 w-[min(18rem,calc(100vw-1.5rem))]';

    return (
      <div
        id="workspace-account-menu"
        data-testid="account-menu-popover"
        className={`${positionClassName} rounded-[1.6rem] border border-white/8 bg-[#1f1f22] p-3 shadow-[0_28px_80px_rgba(0,0,0,0.52)] backdrop-blur`}
      >
  ...
  return (
    <>
      <aside
        className="sticky top-0 z-[70] hidden h-screen w-[92px] shrink-0 flex-col items-center border-r border-white/5 bg-black/25 px-3 py-6 md:flex"
      ...
      >
        ...
        <div className="mt-auto flex flex-col items-center gap-3">
          ...
          <div ref={desktopAccountMenuRef} className="group relative flex flex-col items-center">
            {!accountMenuOpen && (
              <div
            ...
            {accountMenuOpen && renderAccountPopover('desktop')}
          </div>
        </div>
      </aside>

      <div
        className="pointer-events-none fixed inset-x-0 bottom-0 z-[90] px-3 pb-[calc(0.75rem+env(safe-area-inset-bottom))] md:hidden"
        data-testid="mobile-workspace-nav-shell"
      >
        <div className="pointer-events-auto mx-auto flex max-w-md items-center justify-between gap-2 rounded-[1.8rem] border border-white/8 bg-[#151519]/95 px-3 py-2 shadow-[0_22px_50px_rgba(0,0,0,0.35)] backdrop-blur">
          {mobileNavItems.map((item) => {
            ...
            return (
              <Link
                ...
                className={`flex min-w-0 flex-1 flex-col items-center gap-1 rounded-2xl px-2 py-2 text-center text-[11px] font-medium transition ${
                  active
          ...
          <div ref={mobileAccountMenuRef} className="relative flex min-w-0 flex-1 justify-center">
            <button
              type="button"
              aria-label="Open account menu"
              aria-expanded={accountMenuOpen}
            ...
            {accountMenuOpen && renderAccountPopover('mobile')}
          </div>
        </div>
      </div>
    </>
